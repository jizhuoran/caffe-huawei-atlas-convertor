...
class TensorScatter():# pylint: disable=too-many-instance-attributes
    """
       Function: use to store tensor scatter base parameters
       Modify : 2020-05-21
    """

    def __init__(self, # pylint: disable=too-many-arguments,too-many-statements
                 var, indices, updates, var_out, kernel_name, compute_type):
        """
        Init tensor scatter base parameters
        ...
        if self.var_dtype in need_vconv_dtype:
            ...
            if updates_size_bytes * 2 < self.ub_size_bytes * 0.9:
                ...
                self.indices_ub_number = (
                    ...
                    // 2 // self.indices_data_each_block *
                    self.indices_data_each_block)
                self.updates_ub_number = (
    ...
    def cal_data_copy_para(self):
        """
        Cal data num of each core
        ...
        """
        if self.block_num == 1:
            self.var_each_core = self.var_ele_num
        else:
            self.var_each_core = self.var_ele_num // self.block_num
            self.var_each_core = math.ceil(
                self.var_each_core / self.var_data_each_block) \
                * self.var_data_each_block
            self.var_last_core = \
                self.var_ele_num - (self.block_num - 1) * self.var_each_core

        if self.var_dtype in ("float32", "int32"):
            self.max_ub_num = self.indices_ub_number
        elif self.var_dtype in ("uint8", "int8"):
            self.max_ub_num = self.indices_ub_number * 4
        else:
            self.max_ub_num = self.indices_ub_number * 2
    ...
    def updates_the_var(self, indices_in_index, indice_num):
        ...
        None
        """
        indices_burst_len = math.ceil(
            indice_num / self.indices_data_each_block)
        if self.indices_num == 1:
            ...
            self.tik_instance.data_move(self.indices_ub,
                                        self.indices_gm[indices_in_index],
            ...
            if self.block_num > 1:
                with self.tik_instance.if_scope(
                    ...
                    with self.tik_instance.if_scope(
                            ...
                            self.indice_step > self.var_read_index):
                        indices_in_index = \
                            indices_in_index // self.indices_shape[-1]
                ...
                indices_in_index = \
                    indices_in_index // self.indices_shape[-1]
                self.get_updates_read_index(
                    indices_ub_index + indices_in_index)
                self.var_read_index.set_as(self.var_read_index *
                                           self.update_data_num)
                self.calc_updates()
    ...
    def calc_updates_small(self, read_index_offset, element_num):
        ...
        self.tik_instance.data_move(
            self.var_ub, self.out_gm[self.var_read_index + read_index_offset],
        ...
        if (tile_ele_num != 0 and
            ...
            self.tik_instance.data_move(
                self.var_tile_ub,
                self.out_gm[self.var_read_index + align_offset], 0, 1, 1, 0, 0)
    ...
    def calc_process(self, # pylint: disable=too-many-arguments
                     mask, dest_addr, src_addr1, src_addr2, repeat_times,
                     is_tile):
        """
        ...
        if self.var_dtype in need_vconv_dtype:
            if is_tile:
                ...
                self.tik_instance.vconv(mask, "", self.var_vconv_ub[dest_addr],
                                        self.var_ub[src_addr1], repeat_times,
                                        1, 1, 8, 4)
                self.tik_instance.vconv(mask, "",
        ...
        if self.var_dtype in need_vconv_dtype:
            if is_tile:
                self.tik_instance.vconv(mask, "", self.var_tile_ub,
                                        self.var_tile_vconv_ub, repeat_times,
                                        1, 1, 4, 8)
            else:
    ...
    def copy_var_data_to_var_out(self, indices_loop_index):
        """
        Copy var data to out data

        Parameters
        ----------
        indices_loop_index: the index of aicore
        ...
        """
        with self.tik_instance.if_scope(
                indices_loop_index != self.block_num-1):
            var_loop_index = self.var_each_core // self.max_ub_num
            var_last_num = self.var_each_core % self.max_ub_num
            var_burst_len = \
                math.ceil(self.max_ub_num / self.var_data_each_block)
            var_burst_last_len = \
                math.ceil(var_last_num / self.var_data_each_block)
            with self.tik_instance.for_range(0, var_loop_index) as loop_index:
                self.tik_instance.data_move(self.indices_ub,
                                            self.var_gm[indices_loop_index *
                                                        self.var_each_core +
                                                        loop_index *
                                                        self.max_ub_num],
                                            0, 1, var_burst_len, 0, 0)
                self.indices_ub = self.indices_ub.reinterpret_cast_to(
                    self.var_dtype)
                self.tik_instance.data_move(self.out_gm[indices_loop_index *
                                                        self.var_each_core +
                                                        loop_index *
                                                        self.max_ub_num],
                                            self.indices_ub, 0, 1,
                                            var_burst_len, 0, 0)
                self.indices_ub = self.indices_ub.reinterpret_cast_to("int32")
            if var_last_num > 0:
                self.tik_instance.data_move(self.indices_ub, self.var_gm[
                    indices_loop_index * self.var_each_core+var_loop_index *
                    self.max_ub_num], 0, 1, var_burst_last_len, 0, 0)
                self.indices_ub = self.indices_ub.reinterpret_cast_to(
                    self.var_dtype)
                self.tik_instance.data_move(self.out_gm[indices_loop_index *
                                                        self.var_each_core +
                                                        ...
                                                        self.max_ub_num],
                                            self.indices_ub,
                                            0, 1, var_burst_last_len, 0, 0)
                self.indices_ub = self.indices_ub.reinterpret_cast_to("int32")
        with self.tik_instance.else_scope():
            var_left_num = self.var_ele_num - (
                self.block_num-1) * self.var_each_core
            if var_left_num < 0:
                var_left_num = 0
            var_loop_index = var_left_num // self.max_ub_num
            var_last_num = var_left_num % self.max_ub_num
            var_burst_len = math.ceil(
                self.max_ub_num / self.var_data_each_block)
            var_burst_last_len = math.ceil(
                var_last_num / self.var_data_each_block)
            with self.tik_instance.for_range(0, var_loop_index) as loop_index:
                self.tik_instance.data_move(self.indices_ub,
                                            self.var_gm[indices_loop_index *
                                                        self.var_each_core +
    ...
    def tensor_scatter_operator(self):
        """
        Scatter operation
        ...
        if self.block_num > 1:
            with self.tik_instance.for_range(
                ...
                self.copy_var_data_to_var_out(indices_loop_index)
                self.traversing_indices()
        else:
            self.init_ub_tensor()
            self.copy_var_data_to_var_out(0)
            self.traversing_indices()
