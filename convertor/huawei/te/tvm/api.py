...
def negate(*args):
    ...
    """
    if len(args) != 1:
        raise ValueError("Not must take at 1 argument")
    ret = _make.Not(args[0])
    return ret
...
def set_stmt_loc(stmt, loc):
    ...
    """
    _api_internal._SetStmtLoc(stmt, loc)


def get_stmt_loc(stmt):
    """get location information from stmt
    ...
    loc : Loc
        The location information.
    """
    return _api_internal._GetStmtLoc(stmt)


def set_operation_loc(op, loc):
    """set location information for expr
    ...
    loc : Loc
        The location information
    """
    return _api_internal._SetOperationLoc(op, loc)

def get_operation_loc(op):
    """get location information from operation
    ...
    loc : Loc
        The location information.
    """
    return _api_internal._GetOperationLoc(op)

def set_buffer_loc(buffer, loc):
    """set location information for buffer
    ...
    loc : Loc
        The location information
    """
    return _api_internal._SetBufferLoc(buffer, loc)

def get_buffer_loc(buffer):
    """get location information from buffer
    ...
    loc : Loc
        The location information.
    """
    return _api_internal._GetBufferLoc(buffer)
...
def get_current_loc():
    ...
    """
    return _api_internal._GetCurrentLoc()

class LocationManager:
    """ location information manager for IRNode """
    def __init__(self):
        self._loc_dict = {}

    def set_loc(self, node, loc):
        self._loc_dict[node] = loc

    def get_loc(self, node):
        return self._loc_dict[node]
    ...
    def update_loc(self):
        ...
        columns = []
        cnt = 0
        # unpack python tuple to list
        for key in self._loc_dict:
            file, col = self._loc_dict[key]
            loc2num[key] = cnt
            files.append(file)
            columns.append(col)
            cnt += 1
        # clear location dict after update
        self._loc_dict = {}
        return _api_internal._UpdateLoc(loc2num, files, columns)

def location_init():
    # location init
    return _api_internal._LocationInit()
...
def placeholder(shape, dtype=None, name="placeholder", attrs=None):
    ...
    if isinstance(shape, _expr.Expr):
        ...
        shape = [int(x) if not isinstance(x, (int, _expr.Expr)) else x for x in shape]
    dtype = float32 if dtype is None else dtype
    res = _api_internal._Placeholder(shape, dtype, name, attrs)
    build_param.tensor_list.append(res)
    return res
...
def keep_old_value(dtype):
    ...
    """
    return _expr.Call(dtype, "keep_old_value", convert([]), _expr.Call.PureExtern, None, 0)
...
def select(cond, t, f=None):
    ...
    if isinstance(f, type(None)):
        return _expr.Select(convert(cond), convert(t), keep_old_value(t.dtype))
...
prod = comm_reducer(lambda x, y: x*y, lambda t: const(1, dtype=t), name="prod")
bit = comm_reducer(lambda x, y: _make.bitwise_and(x, y) , lambda t: const(0, dtype=t), name='bit')
