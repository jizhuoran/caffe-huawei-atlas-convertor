...
def max_pool(t_x, howo, pad, pooling_params):
    """
    template: window(3, 3), stride(1, 1)
    :param t_x:
    :param howo:
    :param pad:
    ...
    tx_rw1 = tvm.compute(
        shape,
        lambda *i: tvm.max(tx_ub[i[0], i[1], i[2], i[3]*1, i[4]],
                           tx_ub[i[0], i[1], i[2], i[3]*1 + 1, i[4]]),
        name="x_rw1"
    )
    tx_rw2 = tvm.compute(
        shape,
        lambda *i: tvm.max(tx_ub[i[0], i[1], i[2], i[3]*1 + 2, i[4]],
    ...
    tx_rh1 = tvm.compute(
        shape,
        lambda *i: tvm.max(tx_rw2[i[0], i[1], i[2]*1, i[3], i[4]],
                           tx_rw2[i[0], i[1], i[2]*1 + 1, i[3], i[4]]),
        name="x_rh1"
    )
    tx_rh2 = tvm.compute(
        shape,
        lambda *i: tvm.max(tx_rw2[i[0], i[1], i[2]*1 + 2, i[3], i[4]],
    ...
    t_y = tvm.compute(
        ...
        name="pooling2d_res",
        tag=POOL2D_TAG + "max",
        attrs={"pooling_params": pooling_params, "template": "max_3_3_1_1"}
    )
