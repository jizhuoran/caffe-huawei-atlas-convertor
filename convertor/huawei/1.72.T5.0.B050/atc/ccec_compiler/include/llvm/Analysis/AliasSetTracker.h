...
class AliasSet : public ilist_node<AliasSet> {
  ...
  // Purpose of this `public` access declaration is for HiIPUSyncAnalysis
public:
...
private:
  unsigned Access : 2;
  /// Shadow of Access but for LICM + Sync Analysis purpose
  unsigned AccessSA : 2;
...
public:
  ...
  void setAccessSA(unsigned access) { AccessSA = access; }
  unsigned getNumOfUnknownInsts() const { return UnknownInsts.size(); }
...
public:
  ...
  bool isRefSA() const { return AccessSA & RefAccess; }
  bool isModSA() const { return AccessSA & ModAccess; }
...
private:
  ...
  AliasSet()
      : PtrListEnd(&PtrList), RefCount(0), AliasAny(false), Access(NoAccess),
        AccessSA(NoAccess), Alias(SetMustAlias) {}
  ...
  void addUnknownInst(Instruction *I, AliasAnalysis &AA,
                      bool realUnknown = false); // realUnknown helps fall back
                                                 // to the original behavior in
                                                 // case of
                                                 // StrongerThanMonotonic LI &
                                                 // SI
...
public:
  ...
  explicit AliasSetTracker(AliasAnalysis &aa, bool defaultMod = false)
      : AA(aa), OnlyUnknownsDefaultMod(defaultMod) {}
  ...
  void addUnknown(Instruction *I,
                  bool realUnknown = false); // realUnknown helps preserve the
                                             // original behavior in case of
                                             // StrongerThanMonotonic LI & SI
