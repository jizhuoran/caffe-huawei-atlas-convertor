...
class DIDerivedType : public DIType {
  ...
  DIDerivedType(LLVMContext &C, StorageType Storage, unsigned Tag,
                unsigned Line, uint64_t SizeInBits, uint32_t AlignInBits,
                uint64_t OffsetInBits, unsigned AttrType,
                Optional<unsigned> DWARFAddressSpace, DIFlags Flags,
                ArrayRef<Metadata *> Ops)
      : DIType(C, DIDerivedTypeKind, Storage, Tag, Line, SizeInBits,
               AlignInBits, OffsetInBits, Flags, Ops),
        AttrType(AttrType), DWARFAddressSpace(DWARFAddressSpace) {}
  ...
  static DIDerivedType *getImpl(LLVMContext &Context, unsigned Tag,
    ...
    return getImpl(Context, Tag, getCanonicalMDString(Context, Name), File,
                   ...
                   AttrType, DWARFAddressSpace, Flags, ExtraData, Storage,
                   ShouldCreate);
  }
  static DIDerivedType *getImpl(LLVMContext &Context, unsigned Tag,
  ...
  TempDIDerivedType cloneImpl() const {
    return getTemporary(getContext(), getTag(), getName(), getFile(), getLine(),
                        ...
                        getAlignInBits(), getOffsetInBits(), getAttrType(),
...
public:
  DEFINE_MDNODE_GET(DIDerivedType,
                    ...
                    (Tag, Name, File, Line, Scope, BaseType, SizeInBits,
                     AlignInBits, OffsetInBits, AttrType, DWARFAddressSpace,
                     Flags, ExtraData))
  DEFINE_MDNODE_GET(DIDerivedType,
                    (unsigned Tag, StringRef Name, DIFile *File, unsigned Line,
                     DIScopeRef Scope, DITypeRef BaseType, uint64_t SizeInBits,
                     uint32_t AlignInBits, uint64_t OffsetInBits,
                     unsigned AttrType, Optional<unsigned> DWARFAddressSpace,
  ...
  unsigned getAttrType() const { return AttrType; }
...
private:
  ...
  DISubprogram(LLVMContext &C, StorageType Storage, unsigned Line,
               unsigned ScopeLine, unsigned VirtualIndex, unsigned FuncType,
               int ThisAdjustment, DIFlags Flags, DISPFlags SPFlags,
               ArrayRef<Metadata *> Ops)
      : DILocalScope(C, DISubprogramKind, Storage, dwarf::DW_TAG_subprogram,
                     Ops),
        Line(Line), ScopeLine(ScopeLine), VirtualIndex(VirtualIndex),
        FuncType(FuncType), ThisAdjustment(ThisAdjustment), Flags(Flags),
        SPFlags(SPFlags) {
  ...
  getImpl(LLVMContext &Context, DIScopeRef Scope, StringRef Name,
          ...
          unsigned VirtualIndex, unsigned FuncType, int ThisAdjustment,
    ...
    return getImpl(Context, Scope, getCanonicalMDString(Context, Name),
                   ...
                   ScopeLine, ContainingType, VirtualIndex, FuncType,
                   ThisAdjustment, Flags, SPFlags, Unit, TemplateParams.get(),
                   Declaration, RetainedNodes.get(), ThrownTypes.get(), Storage,
                   ShouldCreate);
  }
  static DISubprogram *
  getImpl(LLVMContext &Context, Metadata *Scope, MDString *Name,
          ...
          unsigned FuncType, int ThisAdjustment, DIFlags Flags,
          DISPFlags SPFlags, Metadata *Unit, Metadata *TemplateParams,
          Metadata *Declaration, Metadata *RetainedNodes, Metadata *ThrownTypes,
          StorageType Storage, bool ShouldCreate = true);

  TempDISubprogram cloneImpl() const {
    return getTemporary(getContext(), getScope(), getName(), getLinkageName(),
                        getFile(), getLine(), getType(), getScopeLine(),
                        getContainingType(), getVirtualIndex(), getFuncType(),
...
public:
  DEFINE_MDNODE_GET(
      ...
      (DIScopeRef Scope, StringRef Name, StringRef LinkageName, DIFile *File,
       ...
       DITypeRef ContainingType, unsigned VirtualIndex, unsigned FuncType,
       int ThisAdjustment, DIFlags Flags, DISPFlags SPFlags,
       DICompileUnit *Unit, DITemplateParameterArray TemplateParams = nullptr,
       DISubprogram *Declaration = nullptr, DINodeArray RetainedNodes = nullptr,
       DITypeArray ThrownTypes = nullptr),
      (Scope, Name, LinkageName, File, Line, Type, ScopeLine, ContainingType,
       VirtualIndex, FuncType, ThisAdjustment, Flags, SPFlags, Unit,
  ...
  DEFINE_MDNODE_GET(
      ...
      (Metadata * Scope, MDString *Name, MDString *LinkageName, Metadata *File,
       ...
       Metadata *ContainingType, unsigned VirtualIndex, unsigned FuncType,
...
public:
  ...
  unsigned getFuncType() const { return FuncType; }
...
protected:
  DIVariable(LLVMContext &C, unsigned ID, StorageType Storage, unsigned Line,
      ...
      : DINode(C, ID, Storage, dwarf::DW_TAG_variable, Ops), Line(Line),
        AlignInBits(AlignInBits), AttrType(AttrType) {}
  ~DIVariable() = default;
...
class DILocalVariable : public DIVariable {
  ...
  DILocalVariable(LLVMContext &C, StorageType Storage, unsigned Line,
                  ...
                  unsigned AttrType, ArrayRef<Metadata *> Ops)
  ...
  static DILocalVariable *getImpl(LLVMContext &Context, DIScope *Scope,
                                  ...
                                  uint32_t AlignInBits, unsigned AttrType,
                                  StorageType Storage,
                                  bool ShouldCreate = true) {
    return getImpl(Context, Scope, getCanonicalMDString(Context, Name), File,
                   Line, Type, Arg, Flags, AlignInBits, AttrType, Storage,
                   ShouldCreate);
  }
