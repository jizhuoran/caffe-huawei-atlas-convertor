...
public:
  ...
  bool isTriviallyReMaterializable(const MachineInstr &MI,
    ...
    return MI.getOpcode() == TargetOpcode::IMPLICIT_DEF ||
           (isTargetReMaterializableAvailable() &&
            MI.getDesc().isRematerializable() &&
            (isReallyTriviallyReMaterializable(MI, AA) ||
             isReallyTriviallyReMaterializableGeneric(MI, AA)));
  }

  /// Check whether the given platform want to always hoist trivially
  /// rematerializable instructions regardless of register pressure.
  virtual bool shouldAlwaysHoist(const MachineInstr &MI,
                                 AliasAnalysis *AA = nullptr) const {
    return isTriviallyReMaterializable(MI, AA);
  }

  /// Target defined function to check if there is a dependence
  virtual bool hasDependence(const MachineInstr *MI,
                             const MachineInstr *Barrier) const {
    return true;
  }

protected:
  /// Target defined function to enable or disable rematerialization
  virtual bool isTargetReMaterializableAvailable() const { return true; }
