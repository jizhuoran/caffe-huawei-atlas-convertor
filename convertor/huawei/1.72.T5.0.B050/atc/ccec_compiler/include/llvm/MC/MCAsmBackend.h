...
public:
  ...
  // when the imm of the calling function is small
  // the scene that exceeds imm is placed in the linker to increase the jump
  // scene.
  virtual bool shouldForceRelocation(const MCAssembler &Asm,
                                     const MCFixup &Fixup,
                                     const MCValue &Target,
                                     const uint64_t &Value) {
    return shouldForceRelocation(Asm, Fixup, Target);
  }
