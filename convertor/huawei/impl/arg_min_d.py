...
def arg_min_d(x, y, dimension, kernel_name="arg_min_d"):
    ...
    output_type: int
        dtype of outputdata.Default int64
    kernel_name: str
        ...
        with self.tik_instance.for_range(
            ...
            if core_segment_tail != 0:
                with self.tik_instance.if_scope(n_i < (core_num_used - 1)):
                    self.compute_argmin_last_axis(n_i, core_segment,
                                                  core_segment)
                with self.tik_instance.else_scope():
                    self.compute_argmin_last_axis(n_i, core_segment_tail,
                                                  core_segment)
    ...
    def compute_argmin_last_axis(self, n_i, core_segment, segment_core):
        ...
        def _run(segment_len, segment_index):
            with self.tik_instance.for_range(0, segment_len) as core_i:
                ...
                if self.dtype_x == "float16":
                    argmin_func = self.do_argmin_last_axis
