...
class NmsComputer(ClsProbComputer):
    ...
    def __init__(self, input_dict):
        ...
        if tik.Dprofile().get_unified_buffer_size() // 1024 < 200 \
        ...
        if tik.Dprofile().get_product_name() not in (
                constant.MINI, constant.CLOUD,
                constant.HISI_ES) \
    ...
    def set_class_nms(self, param):
        ...
        repeats = common.get_vector_repeat_times(self.instance,
                                                 param["obj_total"])
        if tik.Dprofile().get_product_name() in (constant.MINI, constant.CLOUD, \
                                                 constant.HISI_ES):
            loop_start = self.instance.Scalar("int32")
    ...
    def nms_filter(self, proposals_selected, selected_tmp, selected_count,
        ...
        if tik.Dprofile().get_product_name() == constant.HISI_ES or \
    ...
    def multi_class(self, batch, image_ub, param):
        ...
        mask = None
        if tik.Dprofile().get_product_name() not in (
                constant.MINI, constant.CLOUD, constant.HISI_ES):
            dtype = "uint16"
            if self.dtype == constant.DATA_TYPE_FP32:
                dtype = "uint32"
        ...
        with self.instance.new_stmt_scope():
            ...
            param["ub_num"] = ub_num
            param["last_ub_num"] = last_ub_num
            if tik.Dprofile().get_product_name() in (
                    constant.MINI, constant.CLOUD, constant.HISI_ES):
