from .proto import caffe_pb2 as caffe_pb2
import google.protobuf.text_format
import json

def process_kernels(kernel_name, layer_name=""):
    kernel_infos = []
    for name in kernel_name:
        with open('kernel_meta/{}.json'.format(name), 'r') as f:
            info = json.load(f)
            kernel_infos.append({'kernelfile' : "{}.o".format(name), 'kernelname' : "{}__kernel0".format(name), "block_num" : info['blockDim']})
    with open('kernel_meta/{}.json'.format(layer_name), 'w') as f:
        json.dump(kernel_infos, f)
...
def gen_net_prototxt(prototxt_path):
    ...
    with open('kernel_meta/{}'.format(new_prototxt_name), 'w') as f:
        f.write(str(net) + '\n')